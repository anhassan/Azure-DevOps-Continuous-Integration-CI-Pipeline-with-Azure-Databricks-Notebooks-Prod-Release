...
# MAGIC %run ../ml_utils/switch_job_ownership
...
def create_jobs(jobs_dict, cluster_name, overwrite=False, switch_ownership = True,
                databricks_instance_name = dbutils.secrets.get(scope="databricks-credentials",key="databricks-instance-name")):
  
  authorization_token = dbutils.secrets.get(scope="idp",key="databricks-authentication-token")
  headers={"Authorization": "Bearer "+authorization_token}
  ...
  for job_name, key in jobs_dict.items():
    ...
    if job_name in existing_jobs:
      print("Job with Job Name :  {} already exists".format(job_name))
    ...
    headers={"Authorization": "Bearer {}".format(dbutils.secrets.get(scope="idp",key="databricks-authentication-token")), 'Content-type': 'application/json'}  
